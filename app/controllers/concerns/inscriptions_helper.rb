...
module InscriptionsHelper
  ...
  def assign_inscription_to_job(job, user, route)
    ...
    if @inscription.save  
      ModelMailer.new_candidate(user, job).deliver_later if if Rails.env.staging?
      ModelMailer.successfully_inscribed(user, job).deliver_later if if Rails.env.staging?
